    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            using (FileStream fs = new FileStream("file.txt", FileMode.OpenOrCreate))
            {
                ...
                byte[] data = Encoding.Unicode.GetBytes("Привіт!");
            ...
            using (FileStream fs = new(path, FileMode.OpenOrCreate, FileAccess.ReadWrite))
            {
                ...
                fs.Write(Encoding.Unicode.GetBytes("Деякий текст для запису в файл у байтовому вигляді"));
            ...
            using (FileStream fs = new("binary.dat", FileMode.OpenOrCreate))
            {
                using (BinaryReader br = new BinaryReader(fs))
                {
                    ...
                    string text = br.ReadString(); // Читання рядка
            ...
            Console.WriteLine(regex.IsMatch(email) ? "Телефон записано." : "Невірний формат номеру!");
